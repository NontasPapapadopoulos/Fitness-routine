...
class UpdateBodyMeasurementTest {

    private lateinit var updateBodyMeasurement: UpdateBodyMeasurement
    ...
    @Before
    fun setUp() {
        updateBodyMeasurement = UpdateBodyMeasurement(bodyMeasurementRepository, dispatcher)
    }

    @Test
    fun execute_updateBodyMeasurement() = runTest {
        whenever(bodyMeasurementRepository.update(any())).thenReturn(Unit)

        val result = updateBodyMeasurement.execute(UpdateBodyMeasurement.Params(
